...
from google_cloud_pipeline_components.experimental.natural_language.convert_dataset_export_for_batch_predict import component as comp
...
class ConvertClassificationExportForBatchPredictTest(parameterized.TestCase):
  ...
  def test_datagen(self, files, classification_type, expected):
    ...
    output_files_tuple = comp.convert_dataset_export_for_batch_predict(
  ...
  def test_load_dataset_non_existent(self):
    ...
    with self.assertRaises(tf.errors.NotFoundError):
      _ = comp.convert_dataset_export_for_batch_predict(
          file_paths=file_paths,
          classification_type='multiclass',
          output_dir=output_dir,
      )
  ...
  def test_datagen_invalid_data_paths(self):
    output_dir = epath.Path(self.create_tempdir().full_path)
    file_paths = [os.path.join(TESTDATA_ROOT, f) for f in INVALID_DATA_FILES]
    with self.assertRaises(tf.errors.NotFoundError):
      _ = comp.convert_dataset_export_for_batch_predict(
