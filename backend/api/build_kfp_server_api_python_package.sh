...
CURRENT_DIR="$(pwd)"
DIR="$CURRENT_DIR/$1/python_http_client"
swagger_file="$CURRENT_DIR/$1/swagger/kfp_api_$1_single_file.swagger.json"

echo "Removing old content in DIR first."
rm -rf "$DIR"

echo "Generating python code from swagger json in $DIR."
java -jar "$codegen_file" generate -g python -t "$CURRENT_DIR/$1/python_http_client_template" -i "$swagger_file" -o "$DIR" -c <(echo '{
    "packageName": "'"kfp_server_api_$1"'",
